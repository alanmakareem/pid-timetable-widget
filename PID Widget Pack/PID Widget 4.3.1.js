...
// PID Timetable Widget (v4.3.1)
// - "Next Arrival" logic updated to group by platform, not destination.
...
const SHOW_NEXT_ARRIVAL_INFO = true; // true to show next arrival info, false to hide it
const DATABASE_FILENAME = "pid_stops_db.json";
const SORT_BY_CLOSEST_STOP_ONLY = false; //true to only show the closest stop, false to show all nearby stops
const WIDGET_VERSION = "4.3.1"; // Version updated to reflect changes
...
async function createWidget(apiResponse, distanceMap, isFallbackSearch, fallbackStopData) {
    ...
    if (isFallbackSearch && fallbackStopData) {
        const headerText = headerStack.addText(`Nearest stops ≈ ${Math.round(fallbackStopData.distance)}m`);
    ...
    dedupedDepartures.forEach(dep => {
        // --- MODIFICATION: Group by line and platform, NOT by destination ---
        const key = `${dep.route.short_name}|${dep.stop.id}`;
        if (!timetable[key]) timetable[key] = [];
        timetable[key].push(new Date(dep.departure_timestamp.predicted));
    });
    Object.keys(timetable).forEach(key => timetable[key].sort((a, b) => a - b));
    ...
    const filteredDepartures = dedupedDepartures.filter(dep => {
        ...
        const buffer = (dep.route.type === 1) ? WALKING_TIME_BUFFER_MINUTES_METRO : WALKING_TIME_BUFFER_MINUTES_DEFAULT;
        const walkingTimeSeconds = distance / AVERAGE_WALKING_SPEED_MPS;
        const requiredTime = Math.ceil(walkingTimeSeconds / 60) + buffer;
        const arrivalTime = new Date(dep.departure_timestamp.predicted);
        const minutesUntil = Math.floor((arrivalTime - now) / (1000 * 60));
        return minutesUntil >= requiredTime;
    });
    ...
    if (sortedDepartures.length === 0) {
        ...
        sortedDepartures.forEach((departure) => {
            ...
            if (departure.trip.is_air_conditioned) {
                ...
                const acText = indicatorsStack.addText("❄︎ AC");
            ...
            if (SHOW_NEXT_ARRIVAL_INFO) {
                // --- MODIFICATION: Use the new key (line + platform) for lookup ---
                const key = `${departure.route.short_name}|${departure.stop.id}`;
                const arrivalTimes = timetable[key] || [];
                const currentIndex = arrivalTimes.findIndex(t => t.getTime() === arrivalTime.getTime());

                if (currentIndex > -1 && currentIndex < arrivalTimes.length - 1) {
                    const nextArrivalTime = arrivalTimes[currentIndex + 1];
                    const isNextDepartureVisible = sortedDepartures.some(dep =>
                        ...
                        dep.stop.id === departure.stop.id // Ensure it's the same platform
                    );
